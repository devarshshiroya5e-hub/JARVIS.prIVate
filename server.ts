...
SYSTEM CONTROL: When user asks to do an action (e.g. open an app, search web, capture screenshot, check metrics, files, etc.), ALWAYS call the matching tool.`;
  ...
  async function generateContentWithRetry(params: any, retries = 3, backoff = 1000): Promise<any> {
    try {
      ...
      if (retries > 0 && (err.status === 503 || err.message?.includes('503') || err.message?.includes('unavailable'))) {
        console.warn(`Gemini 503, retrying in ${backoff}ms... (${retries} attempts left)`);
